...
select results_eq(
  $$select pg_get_functiondef('public.begin_single_credential_generation(uuid,uuid,uuid)'::regprocedure) like '%select template_package.*%'$$,
  $$ values (true)$$,
  'generation begin should expand the Template Package composite before assignment'
);
select results_eq(
