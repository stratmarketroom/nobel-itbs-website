-- AUTH-003 hardening: preserve at least one active Owner and one Owner role.
...
begin
  ...
  if tg_op = 'DELETE' then
    if old.is_owner and old.is_active then
      if not v_actor_is_active_owner then
  ...
  if tg_op = 'INSERT' then
    ...
    if old.is_owner
      and old.is_active
      and (not new.is_owner or not new.is_active) then
      select exists (
        select 1
        from public.user_profiles existing_owner
        where existing_owner.is_active
...
drop trigger if exists user_profiles_enforce_owner_rules on public.user_profiles;
...
declare
  ...
  v_other_owner_role_exists boolean;
...
begin
  ...
  if old.role = 'owner'::public.app_role
    ...
    select exists (
      ...
      from public.user_roles existing_owner_role
      where existing_owner_role.role = 'owner'::public.app_role
        and existing_owner_role.user_id <> old.user_id
    )
    into v_other_owner_role_exists;

    if not v_other_owner_role_exists then
      raise exception 'At least one Owner role assignment is required.'
        using errcode = '23514';
    end if;
  end if;
...
drop trigger if exists user_roles_enforce_owner_rules on public.user_roles;
