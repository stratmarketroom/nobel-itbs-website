-- LRN-LINT-001: Make learner import lintable and safe for repeated calls.
-- The original function staged normalized rows in a transaction-scoped temporary
-- table. Static analysis could not resolve that relation, and a second call in
-- the same transaction failed because ON COMMIT DROP had not run yet.
...
begin
  ...
  select jsonb_agg(
    jsonb_build_object(
      ...
      'latin_first_name', btrim(imported.latin_first_name),
      'latin_last_name', btrim(imported.latin_last_name),
      'ukrainian_full_name', btrim(imported.ukrainian_full_name),
      'email', nullif(lower(btrim(imported.email)), ''),
      'phone', nullif(btrim(imported.phone), ''),
      'has_telegram', coalesce(imported.has_telegram, false),
      'telegram_username', nullif(regexp_replace(btrim(imported.telegram_username), '^@', ''), ''),
      'has_viber', coalesce(imported.has_viber, false),
      'has_whatsapp', coalesce(imported.has_whatsapp, false),
      'internal_note', nullif(btrim(imported.internal_note), '')
    )
  ...
  if v_normalized_rows is null or jsonb_array_length(v_normalized_rows) <> v_count then
    raise exception 'Learner import rows could not be parsed.' using errcode = '22023';
  end if;

  if exists (
    select 1
    from jsonb_to_recordset(v_normalized_rows) as imported(
      row_number integer,
      latin_first_name text,
      latin_last_name text,
      ukrainian_full_name text,
    ...
    where row_number is null or row_number < 2
      or latin_first_name is null or latin_first_name = '' or char_length(latin_first_name) > 160
      or latin_last_name is null or latin_last_name = '' or char_length(latin_last_name) > 160
      or ukrainian_full_name is null or ukrainian_full_name = '' or char_length(ukrainian_full_name) > 320
  ...
  if exists (
    ...
    join public.learner_emails existing on existing.email = imported.email
  ) or exists (
    select 1
    from jsonb_to_recordset(v_normalized_rows) as imported(
      row_number integer,
    ...
    join public.learner_phones existing on existing.phone = imported.phone
  ) then
...
comment on function public.import_learners(jsonb) is
  'Atomically imports a validated learner batch for an MFA-verified Owner, Super Admin, or Credential Manager. Normalized rows remain transaction-local JSONB; audit metadata contains only the row count.';
