...
export async function generateMetadata({ params }: LocaleSystemPageProps): Promise<Metadata> {
  ...
  return {
    title: `${systemCopy[locale]['rate-limit'].seoTitle} | Nobel ITBS`,
...
export default async function LocaleRateLimitPage({ params }: LocaleSystemPageProps) {
  const { locale } = await params;
  ...
  return <SystemPage copy={systemCopy[locale]['rate-limit']} />;
}
