...
import type { ProgrammeMasterPageCopy } from '@/lib/programme-master-copy';

type ProgrammeMasterPageProps = {
  copy: ProgrammeMasterPageCopy;
  locale: Locale;
};

const sectionLabels: Record<Locale, { value: string; audience: string; outcomes: string; faq: string }> = {
  en: { value: 'Programme value', audience: 'Audience', outcomes: 'Outcomes', faq: 'Questions before applying' },
  ua: { value: 'Цінність програми', audience: 'Аудиторія', outcomes: 'Результати', faq: 'Запитання перед участю' },
  cz: { value: 'Hodnota programu', audience: 'Publikum', outcomes: 'Výsledky', faq: 'Otázky před přihlášením' },
};

function ProgrammeBlocks({ blocks }: { blocks: ProgrammeMasterPageCopy['curriculum']['blocks'] }) {
  return (
    <>
...
export function ProgrammeMasterPage({ copy, locale }: ProgrammeMasterPageProps) {
  ...
  return (
    <main className="programme-detail-page programme-master-page">
      ...
      <section className="programme-value-section section-band" aria-labelledby="value-title">
        <div className="editorial-heading">
          <p className="eyebrow dark">{labels.value}</p>
      ...
      <section className="programme-two-column section-band">
        <article>
          <p className="eyebrow dark">{labels.audience}</p>
        ...
        <article>
          <p className="eyebrow dark">{labels.outcomes}</p>
      ...
      <section className="curriculum-section programme-master-curriculum" aria-labelledby="curriculum-title">
        <div className="curriculum-heading">
          <p className="eyebrow">{copy.curriculum.heading}</p>
          <h2 id="curriculum-title">{copy.curriculum.heading}</h2>
        </div>
        <div className="master-content-blocks">
          <ProgrammeBlocks blocks={copy.curriculum.blocks} />
        </div>
      </section>

      <section className="master-section-list section-band" aria-label="Programme details">
        {copy.sections.map((section) => (
          <article key={section.heading}>
            <h2>{section.heading}</h2>
            <ProgrammeBlocks blocks={section.blocks} />
          </article>
        ))}
      </section>
