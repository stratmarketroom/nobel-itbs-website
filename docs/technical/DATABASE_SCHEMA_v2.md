...
- `credential_generation_batch_activation_requests` — private idempotent
  aggregate ledger for one exact explicitly reviewed selection;
- `credential_generation_batch_activation_items` — private per-credential
  activation/delivery outcome, lease, retry, and immutable email-history link
  without recipient, message, file bytes, or private paths.

PDFGEN-005 additionally uses
`internal.credential_single_generation_locks` as a short-lived, forced-RLS,
no-policy lease for one pending credential. It is an internal concurrency
control rather than product data: successful completion or an explicit safe
failure removes it, and a later guarded start reclaims an expired lease while
recording a bounded failure outcome.
