...
41. [-] **Implement automatic credential document generation** — PDFGEN-001, PDFGEN-002, and PDFGEN-003 are merged, deployed, applied, and accepted at their documented dev/Production level. PDFGEN-003's cleaned real-source v1 is immutably published/read-only in dev; PR #33 merged as `6450fb6`; Preview and Production deployments passed; migration `20260825140000` is applied/recorded in both environments; and Production passed the 57-migration/function/RLS/grant/zero-fixture audit, Owner/AAL2 empty-workspace acceptance, and unauthenticated API `401` smoke. Continue one ticket at a time with PDFGEN-004 through PDFGEN-008. After that, attach and verify the canonical domain, configure analytics/consent, complete Safari/physical-device/assistive-technology acceptance, and make the final release decision. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_003_TEMPLATE_PACKAGE_ADMIN_FIELD_EDITOR_2026-08-25.md`.
