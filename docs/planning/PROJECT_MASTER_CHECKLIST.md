...
Stage status: **implemented at the current dev level except Owner-deferred WF-004 resend and the real VEDOS SMTP delivery acceptance; WF-001..003, WF-005..008, and the manager workspace are complete**.
...
- [ ] QA-005 production environment, approved credential delivery provider, Leeloo, Telegram contact alert, analytics, backup, launch checklist, and confirmation that conditional CAPTCHA remains disabled unless abuse signals require it.
  ...
  - [-] Align the approved VEDOS SMTP decision with the v2 documents and WF-003 implementation, then acceptance-test real credential delivery. Active documentation, code, provider mailbox, aliases, bidirectional Webmail delivery, and SPF/DKIM/DMARC are aligned; encrypted Vercel secrets, Vercel-origin transport smoke, and a real approved credential delivery remain.
...
22. Before launch, complete PCE-005 Telegram manager notifications; configure encrypted VEDOS SMTP deployment secrets and acceptance-test credential PDF delivery; revisit conditional CAPTCHA only if abuse signals justify enabling it.
