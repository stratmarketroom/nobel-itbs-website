...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 private template/generation database foundation and PDFGEN-002 private source-PDF Storage/validation are merged, deployed, and applied in dev/Production. PDFGEN-002 passed local code/build checks plus independent dev/Production read-only security audits with a private PDF-only 20 MB template bucket, strict server PDF rejection boundary, parsed multi-page metadata/hash, atomic draft attachment/deletion, compensating rollback, private proxied preview, safe column grants, publication object guard, published-object immutability, and zero template fixtures. Continue with PDFGEN-003 Template Package Admin and Field Placement Editor, including the first authenticated real-template workflow acceptance, then one ticket at a time through PDFGEN-008. After that, attach and verify the canonical domain, configure analytics/consent, complete Safari/physical-device/assistive-technology acceptance, and make the final release decision. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_001_TEMPLATE_GENERATION_DATABASE_FOUNDATION_2026-08-25.md` and `docs/qa/PDFGEN_002_PRIVATE_TEMPLATE_STORAGE_VALIDATION_2026-08-25.md`.
