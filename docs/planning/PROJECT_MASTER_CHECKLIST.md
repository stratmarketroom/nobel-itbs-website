...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 private template/generation database foundation is merged and applied in dev/Production. PDFGEN-002 is code-complete and its dev migration/read-only security audit passed with a private PDF-only 20 MB template bucket, strict server PDF rejection boundary, parsed multi-page metadata/hash, atomic draft attachment/deletion, compensating rollback, private proxied preview, safe column grants, publication object guard, published-object immutability, and zero template fixtures; Preview/Production integration plus authenticated live acceptance remain before starting PDFGEN-003. Continue one ticket at a time through PDFGEN-008. Then attach and verify the canonical domain, configure analytics/consent, complete Safari/physical-device/assistive-technology acceptance, and make the final release decision. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_001_TEMPLATE_GENERATION_DATABASE_FOUNDATION_2026-08-25.md` and `docs/qa/PDFGEN_002_PRIVATE_TEMPLATE_STORAGE_VALIDATION_2026-08-25.md`.
