...
- [x] QA-005-SEO-CUTOVER-001 completed the known current-Tilda legacy map at code/local-smoke level on 2026-08-30: `/tracks` now redirects to `/programmes`, `/works` to `/about`, and both preserve query parameters through direct `301` responses. Preview and canonical-domain edge acceptance remain operational launch work.
...
46. [x] **QA-005-SEO-CUTOVER-001 Legacy URL completion** — the current live Tilda sitemap exposed `/tracks` and `/works` after the original inventory. Direct query-preserving `301` mappings to `/programmes` and `/about`, focused regression guards, build, and local HTTP smoke pass. Preview and canonical-domain edge acceptance remain. See `docs/qa/QA_005_SEO_CUTOVER_2026-08-30.md`.
