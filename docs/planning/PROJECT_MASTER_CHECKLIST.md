...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 through PDFGEN-004 are merged, deployed, and accepted at their documented dev/Production level. PDFGEN-004's server-side renderer produced a primary PDF plus a two-page Supplement with embedded mixed EN/UA/CZ text, exact normal/rotated QR round trips, preserved page geometry/orientation, and safe overflow/malformed-source failures. PR #35 passed 2/2 checks and Vercel Preview acceptance, merged as `d777f6c`, and passed the merge commit's 1/1 Production deployment check plus focused public Production smoke without database or Supabase mutation. Continue one ticket at a time with PDFGEN-005 through PDFGEN-008. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_003_TEMPLATE_PACKAGE_ADMIN_FIELD_EDITOR_2026-08-25.md` and `docs/qa/PDFGEN_004_SERVER_SIDE_MULTI_DOCUMENT_GENERATION_2026-08-25.md`.
