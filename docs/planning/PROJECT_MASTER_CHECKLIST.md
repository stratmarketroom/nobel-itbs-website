...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 through PDFGEN-004 are merged, deployed, and accepted at their documented dev/Production level. PDFGEN-005 is complete locally: one pending credential can generate every configured private PDF from an exact matching published package, regenerate the same immutable version with current-file identity preservation, append provenance, recover Storage on failure, and use controlled MFA-protected Preview/Download. Its migration, live mutation acceptance, and publication remain pending; PDFGEN-006 has not started. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_004_SERVER_SIDE_MULTI_DOCUMENT_GENERATION_2026-08-25.md` and `docs/qa/PDFGEN_005_SINGLE_CREDENTIAL_GENERATION_2026-08-26.md`.
