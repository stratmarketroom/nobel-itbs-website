...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 through PDFGEN-006 are merged, deployed, and accepted at their documented dev/Production level. PDFGEN-006 PR #39 merged as `f59c4a0`; Production deployment `A6twmGaa7SoCmRko1oWxqzZYytLi`, migrations 59/60, independent security/read-only audit, canonical Home smoke, and MFA-verified protected batch-workspace smoke pass without consuming a number or creating a credential, PDF, batch, item, or provenance row. Full PDFGEN-006 mutation acceptance remains deferred until an explicitly approved non-production cohort exists; PDFGEN-007 activation/delivery is next. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_005_SINGLE_CREDENTIAL_GENERATION_2026-08-26.md` and `docs/qa/PDFGEN_006_BATCH_GENERATION_REVIEW_2026-08-26.md`.
