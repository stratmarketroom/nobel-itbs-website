...
41. **Implement automatic credential document generation** — execute PDFGEN-001..008 one ticket at a time, beginning with the private template/generation database foundation. Then attach and verify the canonical domain, configure analytics/consent, complete Safari/physical-device/assistive-technology acceptance, and make the final release decision. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance.
