...
- [x] QA-005-SEO-CUTOVER-001 completed the known current-Tilda legacy map on 2026-08-30: `/tracks` now redirects to `/programmes`, `/works` to `/about`, and both preserve query parameters through direct `301` responses. Local HTTP and Ready Vercel Preview browser acceptance passed with the expected canonical destinations; canonical-domain edge acceptance remains operational launch work.
...
- [ ] QA-005 production environment, approved credential delivery provider, Leeloo, analytics, backup, launch checklist, and confirmation that conditional CAPTCHA remains disabled unless abuse signals require it. Telegram contact alerts are complete.
  ...
  - [-] Configure final CTA decisions, analytics/consent, database plus private-PDF backups, canonical domain, and final responsive/accessibility/browser acceptance. The current Owner-approved CTA values and fallbacks are accepted in Production; analytics, backups, domain, and final broad acceptance remain. The backup runbook/static guard is complete; live backup and restore evidence is still required.
