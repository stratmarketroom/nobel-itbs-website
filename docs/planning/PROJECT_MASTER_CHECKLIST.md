...
| Credential Types / Sets / Credentials / Number Log | Credential Type schema, EN/UA/CZ reference labels, deactivation, RLS/MFA, and role matrix implemented | CRD-002..006 and WF sequence |
...
Stage status: **in progress; CRD-001 complete, CRD-002 next**.
...
- [-] QA-001 live Stage 2–4 matrix, complete Stage 5 learner RLS/API/UI, and CRD-001 reference-data RLS passed; future credential tables and the Docker-dependent full pgTAP suite remain.
- [ ] QA-002 credential verification privacy tests after credentials exist.
- [-] QA-003 MFA matrix passed for current admin/content/programme/contact/settings, complete learner management, and CRD-001 type access/mutation; future credential actions remain.
...
- [x] One ordered chain of 32 local migrations matches the remote dev history.
...
8. **Continue Stage 6 with CRD-002 Credential Sets**.
9. Before launch, complete PCE-005 Telegram manager notifications; configure Google Workspace separately only for credential PDF delivery; revisit conditional CAPTCHA only if abuse signals justify enabling it.
10. Run the full automated pgTAP suite when Docker or another compatible runner is available; this remains an infrastructure check, not a blocker for the accepted manager, contact, learner, and credential-type foundation layers.
