...
- [ ] QA-005 production environment, approved credential delivery provider, Leeloo, Telegram contact alert, analytics, backup, launch checklist, and confirmation that conditional CAPTCHA remains disabled unless abuse signals require it.
  ...
  - [-] Configure final CTA decisions, analytics/consent, database plus private-PDF backups, canonical domain, and final responsive/accessibility/browser acceptance. The approved CTA values/fallbacks are now applied at database level; `QA-005-CTA-002` deployment acceptance remains. The backup runbook/static guard is complete; live backup and restore evidence is still required.
...
36. [-] **QA-005 Space Business and For Organisations CTA decision** — the Space Business Leeloo URL is approved and applied to dev and Production through a guarded migration; For Organisations intentionally retains the on-site organisation enquiry form. Complete Preview and post-merge Production acceptance. See `docs/qa/QA_005_SPACE_BUSINESS_AND_B2B_CTA_ACCEPTANCE_2026-08-24.md`.
