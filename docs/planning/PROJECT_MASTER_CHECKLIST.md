...
41. [-] **Implement automatic credential document generation** — PDFGEN-001, PDFGEN-002, and PDFGEN-003 are merged, deployed, applied, and accepted at their documented dev/Production level. PDFGEN-004 is implemented and locally accepted: its server-side renderer produced a primary PDF plus a two-page Supplement with embedded mixed EN/UA/CZ text, exact normal/rotated QR round trips, preserved page geometry/orientation, and safe overflow/malformed-source failures; it has no database or Production mutation. Continue one ticket at a time with PDFGEN-005 through PDFGEN-008 after the normal PDFGEN-004 PR/Preview/merge path. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_003_TEMPLATE_PACKAGE_ADMIN_FIELD_EDITOR_2026-08-25.md` and `docs/qa/PDFGEN_004_SERVER_SIDE_MULTI_DOCUMENT_GENERATION_2026-08-25.md`.
