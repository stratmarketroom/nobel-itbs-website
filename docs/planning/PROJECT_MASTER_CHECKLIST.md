...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 through PDFGEN-005 are merged, deployed, and accepted at their documented dev/Production level. PDFGEN-005 PR #37 merged as `159916c`; Production deployment `2FXVz7Xrz5jegFXnMrRvTwiv3Y5D`, migration 58, independent security/read-only audit, public root smoke, and unauthenticated API `401` smoke pass without consuming a number or creating a template, credential, PDF, batch, or provenance row. PDFGEN-006 is unblocked but has not started. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_004_SERVER_SIDE_MULTI_DOCUMENT_GENERATION_2026-08-25.md` and `docs/qa/PDFGEN_005_SINGLE_CREDENTIAL_GENERATION_2026-08-26.md`.
