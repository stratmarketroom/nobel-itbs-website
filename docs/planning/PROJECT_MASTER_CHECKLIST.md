...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 private template/generation database foundation is implemented, merged through PR #29 as `1b2d224`, and applied in dev and Production with 8 forced-RLS tables, immutable published multi-document/multi-page templates, unbounded-cohort/resumable batch state, append-only provenance, and zero Production fixture rows. Continue one ticket at a time with PDFGEN-002..008, beginning with private template Storage and source-PDF validation. Then attach and verify the canonical domain, configure analytics/consent, complete Safari/physical-device/assistive-technology acceptance, and make the final release decision. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_001_TEMPLATE_GENERATION_DATABASE_FOUNDATION_2026-08-25.md`.
