...
41. [-] **Implement automatic credential document generation** — PDFGEN-001 through PDFGEN-004 are merged, deployed, and accepted at their documented dev/Production level. PDFGEN-005 is complete in dev: PR #37 passes 2/2 checks, Preview is Ready, migration 58 and the independent security audit pass, and authenticated Owner/AAL2 read-only acceptance confirms the pending-only UI boundary without consuming a number or creating a PDF. Merge and Production promotion remain pending; PDFGEN-006 has not started. Real VEDOS acceptance and backup activation/restore remain deferred until real credentials/data exist but are required before operational issuance. See `docs/qa/PDFGEN_004_SERVER_SIDE_MULTI_DOCUMENT_GENERATION_2026-08-25.md` and `docs/qa/PDFGEN_005_SINGLE_CREDENTIAL_GENERATION_2026-08-26.md`.
