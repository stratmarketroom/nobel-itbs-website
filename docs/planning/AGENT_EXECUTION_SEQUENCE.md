...
- `PDFGEN-007 Batch Activation and VEDOS Delivery` — implemented on `codex/pdfgen-007-batch-activation` with explicit reviewed-item selection, exact idempotency, bounded resumable leases, independent activation/delivery outcomes, complete-package VEDOS delivery, forced-RLS private ledgers, safe per-item retry, and protected admin controls. Focused static verification, PDFGEN-006 regression, TypeScript, lint, production build, and diff checks pass. Development migration, Preview publication, and read-only environment acceptance remain pending explicit publication approval; no credential or email was mutated during local QA.
...
- PDFGEN-007 implementation is locally complete and verified. The Owner must explicitly approve branch publication before development migration and hosted read-only acceptance; real activation/delivery remains gated on an approved non-production cohort;
