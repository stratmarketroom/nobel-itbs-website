...
- WF-004 resend implementation is tracked on branch `codex/wf-004-resend-credential`.
...
The repository now contains 53 local SQL migrations. WF-004 migration `20260824130000_wf_004_resend_credential.sql` is applied and recorded in dev; Production remains on the preceding 52 migrations until Preview acceptance. The separate `nobel-itbs-prod` project is in Frankfurt; its secrets remain outside Git.
...
- On 2026-08-24, WF-004 passed all 69 non-live static verifiers, ESLint, TypeScript, production build, and diff checks. Its migration compiled, was applied and recorded in dev, and passed read-only function/grant/lifecycle checks. PR #22 Vercel Preview passed deployment and signed-out protection. The 24-assertion focused pgTAP suite is present but dev lacks the pgTAP `plan(integer)` function; authenticated valid-record Preview and real-delivery checks remain. See `docs/qa/WF_004_RESEND_CREDENTIAL_QA_2026-08-24.md`.
...
- WF-003..008 static verifiers, ADM-CRD-001/PCE-004 regressions, TypeScript, ESLint, and production build pass. WF-004 is promoted to dev but not Production.
...
WF-004 is now applied and recorded in dev. The complete local pgTAP suite was not executed because the Supabase CLI database test command requires Docker and no Docker-compatible runtime is available; dev also lacks the pgTAP `plan(integer)` function. This remains an explicit QA item, not a hidden pass.
...
- PCE-005 Telegram contact alerts are accepted in Production. The dedicated bot, private manager supergroup, and admin origin use encrypted Preview/Production values; VEDOS SMTP remains reserved for credential delivery and is not used for contact alerts. The setup-time private-group invitation link still requires revocation/replacement as operational security hygiene, without affecting the accepted transport path.
...
- Stage 5 Learner Foundation and Stage 6 Credential Core are complete. Stage 7 now has WF-001..008 and ADM-CRD-001 implemented at the documented code level; WF-004 is migrated in dev and passed signed-out Preview protection, but still needs authenticated valid-record smoke plus an Owner-approved real resend before operational acceptance.
...
- Remaining pre-launch work is operational: one approved real VEDOS credential-delivery acceptance, revocation/replacement of the setup-time Telegram group invitation link, analytics/consent, activation and drill of database/private-PDF backups, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Conditional CAPTCHA remains disabled unless abuse signals justify it.
