...
- Current focused branch: `codex/pce-005-production-acceptance`.
...
- PCE-005 is accepted in Production: the dedicated bot and private manager supergroup use sensitive Vercel variables for Production/Preview; all enquiry types, all five programmes, and EN/UA/CZ delivered privacy-minimal alerts. An invalid pre-upgrade group ID confirmed that Telegram failure does not reject a stored submission. VEDOS SMTP remains reserved for credential delivery and is not used for contact alerts.
...
- Telegram contact alerts are accepted in Production under PCE-005: encrypted configuration, successful delivery, programme/locale coverage, privacy projection, and the non-blocking failure path passed. Credential PDF delivery is aligned in code and active documentation with the Owner-approved VEDOS SMTP path; Preview transport, Production configuration, merge, and deployment acceptance passed. One approved real credential delivery remains. CAPTCHA provider setup is deferred by product decision and is not a blocker.
...
- Remaining pre-launch work is operational: one approved real VEDOS credential-delivery acceptance, final CTA destinations, analytics/consent, database and private-PDF backup coverage, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Telegram PCE-005 is complete. Conditional CAPTCHA remains disabled unless abuse signals justify it.
