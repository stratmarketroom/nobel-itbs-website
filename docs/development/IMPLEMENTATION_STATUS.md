...
- `QA-005-CTA-002` assigns the Owner-approved Space Business Leeloo destination while preserving the For Organisations on-site form decision. The guarded migration is applied to dev and Production with clean second dry-runs and exact anonymous read-back. Preview and post-merge Production browser acceptance remain before closure.
- Remaining pre-launch work is operational: one approved real VEDOS credential-delivery acceptance, Telegram secret configuration and transport acceptance, `QA-005-CTA-002` deployment acceptance, analytics/consent, activation and drill of database/private-PDF backups, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Conditional CAPTCHA remains disabled unless abuse signals justify it.
