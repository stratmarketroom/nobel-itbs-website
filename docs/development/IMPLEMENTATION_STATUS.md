...
- Telegram contact-alert delivery remains a deferred pre-launch check. Credential PDF delivery is aligned in code and active documentation with the Owner-approved VEDOS SMTP path; Preview transport, Production configuration, merge, and deployment acceptance passed. One approved real credential delivery remains. CAPTCHA provider setup is deferred by product decision and is not a blocker.
...
- Remaining pre-launch work is operational: one approved real VEDOS credential-delivery acceptance, Telegram PCE-005, final CTA destinations, analytics/consent, database and private-PDF backup coverage, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Conditional CAPTCHA remains disabled unless abuse signals justify it.
