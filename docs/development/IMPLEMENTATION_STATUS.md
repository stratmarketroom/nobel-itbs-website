...
- WF-003..008 static verifiers, ADM-CRD-001/PCE-004 regressions, TypeScript, ESLint, and production build pass. The WF-004 database migration is promoted to dev and Production; application merge/deployment is tracked by PR #22.
...
- Stage 5 Learner Foundation and Stage 6 Credential Core are complete. Stage 7 now has WF-001..008 and ADM-CRD-001 implemented at the documented code level; WF-004 is migrated in dev and Production and passed signed-out Preview protection, but still needs authenticated valid-record smoke plus an Owner-approved real resend when a valid credential exists.
