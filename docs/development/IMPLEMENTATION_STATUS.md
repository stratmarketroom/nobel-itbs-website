...
- `QA-005-EMAIL-001` aligns the active v2 documents and WF-003 code with the Owner-approved VEDOS SMTP mailbox `documents@nobel-itbs.eu`. Provider-side bidirectional Webmail delivery, aliases, inbox placement, and SPF/DKIM/DMARC passed on 2026-08-13. Encrypted Vercel secrets, a Vercel-origin transport smoke, and the first approved real credential delivery remain operational acceptance dependencies.
...
- Telegram contact-alert delivery remains a deferred pre-launch check. Credential PDF delivery is aligned in code and active documentation with the Owner-approved VEDOS SMTP path; encrypted deployment configuration and real delivery acceptance remain. CAPTCHA provider setup is deferred by product decision and is not a blocker.
...
- Remaining pre-launch work is operational: VEDOS Vercel transport plus real credential-delivery acceptance, Telegram PCE-005, final CTA destinations, analytics/consent, database and private-PDF backup coverage, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Conditional CAPTCHA remains disabled unless abuse signals justify it.
