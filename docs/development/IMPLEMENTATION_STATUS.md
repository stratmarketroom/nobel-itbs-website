...
- General Psychology and Child Psychology have accepted Leeloo URLs in dev and Production. The Owner-approved Space Business destination `https://event.duan.edu.ua/et6naw` is also applied to dev and Production with clean second dry-runs and exact anonymous read-back; deployment acceptance is tracked under `QA-005-CTA-002`.
...
- The PCE-005 one-way Telegram adapter is implemented. A dedicated bot, private manager-chat ID, and deployed admin origin still require encrypted Preview/Production configuration and real transport acceptance. VEDOS SMTP remains reserved for credential delivery and is not used for contact alerts.
...
- Telegram contact-alert code is aligned with PCE-005; encrypted deployment configuration and real transport/failure-path acceptance remain. Credential PDF delivery is aligned in code and active documentation with the Owner-approved VEDOS SMTP path; Preview transport, Production configuration, merge, and deployment acceptance passed. One approved real credential delivery remains. CAPTCHA provider setup is deferred by product decision and is not a blocker.
...
- `QA-005-CTA-002` assigns the Owner-approved Space Business Leeloo destination while preserving the For Organisations on-site form decision. The guarded migration is applied to dev and Production with clean second dry-runs and exact anonymous read-back. PR #19 Preview acceptance passed across EN/UA/CZ for both paths; post-merge Production browser acceptance remains before closure.
- Remaining pre-launch work is operational: one approved real VEDOS credential-delivery acceptance, Telegram secret configuration and transport acceptance, `QA-005-CTA-002` post-merge Production acceptance, analytics/consent, activation and drill of database/private-PDF backups, canonical-domain verification, and final physical-device/accessibility/cross-browser acceptance. Conditional CAPTCHA remains disabled unless abuse signals justify it.
