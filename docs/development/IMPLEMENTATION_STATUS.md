...
- The Owner-approved Home visual baseline is preserved at commit `24f728b` and documented in `docs/design/HOME_VISUAL_BASELINE.md`. Its visual layer still must be ported into the current structured Supabase-backed Home before final responsive/accessibility acceptance; the rejected PR #3 recovery must not be merged.
...
- Next integrate the approved Home visual baseline into the current architecture, then continue the existing Vercel production configuration without connecting the public domain. Production Auth/Owner/MFA, backup coverage, full responsive/accessibility/browser QA, and external integrations still remain before launch.
