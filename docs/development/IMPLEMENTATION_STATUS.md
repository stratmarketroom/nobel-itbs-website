...
- `QA-005-CTA-001` assigns the Owner-approved Leeloo destinations to General Psychology and Child Psychology through a guarded forward-only migration. The migration is applied to dev and Production with clean second dry-runs and exact anonymous read-back. AI Production intentionally retains the on-site question fallback; no active-run or pricing URL override is introduced. Deployment browser acceptance remains before ticket closure.
