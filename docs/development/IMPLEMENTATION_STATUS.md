...
- General Psychology and Child Psychology now have the Owner-approved Leeloo URLs in dev and Production; PR #17 Preview acceptance passed across EN/UA/CZ. Space Business still requires its final destination.
...
- `QA-005-CTA-001` assigns the Owner-approved Leeloo destinations to General Psychology and Child Psychology through a guarded forward-only migration. The migration is applied to dev and Production with clean second dry-runs and exact anonymous read-back. PR #17 Preview acceptance passed across EN/UA/CZ: both approved destinations render twice per page with safe external attributes, the question path remains available, and AI Production has no external event URL. Post-merge Production browser acceptance remains before ticket closure.
