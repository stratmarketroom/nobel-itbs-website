...
- On 2026-08-24, WF-004 passed all 69 non-live static verifiers, ESLint, TypeScript, production build, and diff checks. Its migration compiled, was applied and recorded in dev, and passed read-only function/grant/lifecycle checks. The 24-assertion focused pgTAP suite is present but dev lacks the pgTAP `plan(integer)` function; protected Preview and real-delivery checks remain. See `docs/qa/WF_004_RESEND_CREDENTIAL_QA_2026-08-24.md`.
...
- Stage 5 Learner Foundation and Stage 6 Credential Core are complete. Stage 7 now has WF-001..008 and ADM-CRD-001 implemented at the documented code level; WF-004 is migrated in dev and still needs protected Preview smoke plus an Owner-approved real valid-credential resend before operational acceptance.
