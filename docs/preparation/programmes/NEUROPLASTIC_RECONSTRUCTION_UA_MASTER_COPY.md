...
- Старт поточного набору: 5 жовтня
...
`cta_note`: До підключення окремого шляху оплати Nobel ITBS використовує контактний CTA без перенесення зовнішнього checkout URL.
...
`body`: Поставте запитання про програму, формат навчання та умови участі.
