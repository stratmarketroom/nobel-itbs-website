...
`photo_status`: source received  
`profile_status`: name, short title, photograph and General Psychology lecturer role approved  

Received source asset:

| Source asset | Format | Dimensions | Colour profile | Technical status |
| --- | --- | ---: | --- | --- |
| `assets/source/experts/alina-yudina/portrait-original.jpeg` | JPEG | 4000 × 6000 | sRGB | Suitable high-resolution portrait source |

Recommended production treatment:

- preserve the supplied original unchanged;
- create a cropped and compressed sRGB WebP or AVIF derivative for expert cards;
- use localized alt text `Аліна Юдіна`, `Alina Yudina`, and `Alina Yudina`;
- the received photo and current concise profile are approved for Release 1.
