# For Organisations: CZ Master Copy

Product: Nobel ITBS Website and Credential Registry  
Page: For Organisations  
Locale: Czech  
URL: `/cz/for-organisations`  
Status: localized from product-owner-approved UA master copy; native-language and parity QA pending  
Updated: 2026-07-31

## SEO
`seo_title`: Vzdělávací infrastruktura pro online školy | Nobel ITBS
`seo_description`: Infrastrukturní služby Nobel ITBS pro online školy a experty: strukturování programů, dokumenty a dodatky, registrace a ověřování.
`og_title`: Infrastruktura Nobel ITBS pro vzdělávací projekty
...
`step_2_body`: Určíme, co strukturovat, jaké dokumenty jsou vhodné a jak proběhne jejich příprava, registrace a ověření.
`step_3_title`: Dohodneme model
`step_3_body`: Stanovíme rozsah, role, požadavky na data a pravidla použití dokumentů.
`step_4_title`: Podpoříme realizaci
...
Model práv se sjednává samostatně; infrastrukturní spolupráce automatický převod neznamená.
### Může každý program získat univerzitní certifikát?
Ne. Záleží na obsahu, rozsahu, výsledcích, hodnocení a dohodnutém modelu.
### Lze využít jen registraci a ověřování?
Rozsah služeb se určí po posouzení programu a stávajícího systému dokumentů.

## Final CTA
`heading`: Vybudujme infrastrukturu pro váš vzdělávací program
`copy`: Popište projekt, současný formát a plánované dokumenty. Po úvodním posouzení navrhneme další krok.
`primary_cta`: Projednat projekt
