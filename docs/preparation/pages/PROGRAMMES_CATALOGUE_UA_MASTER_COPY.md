...
`status_detail`: Старт поточного набору — 5 жовтня 2026 року
...
`document`: Сертифікат Університету імені Альфреда Нобеля; години на сертифікаті не зазначаються.
...
- continue EN and CZ native-language QA after publication where useful;
